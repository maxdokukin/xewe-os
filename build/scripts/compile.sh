...
ESP_CHIP="" PROJECT_ROOT="" BUILDS_DIR="" WORK_DIR="" TARGET_DIR=""
...
[[ -z "$ESP_CHIP" || -z "$PROJECT_ROOT" || -z "$BUILDS_DIR" || -z "$WORK_DIR" || -z "$TARGET_DIR" || -z "$PROJECT_NAME" || -z "$VERSION_NEXT" || -z "$TS_ISO" ]] && usage_fail "Missing required arguments."
