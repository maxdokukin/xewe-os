...
The XeWe OS functions through independent modules. Below is a detailed breakdown of each module and its available commands.
...
| **`add`** | Add a button mapping.<br>

<br>**Args:** `<pin> "<cmd>" [mode] [trigger] [debounce]`<br>

<br>**Modes:** `pullup`, `pulldown`<br>

<br>**Triggers:** `on_press`, `on_release`, `on_change` | `$buttons add 9 "$system reboot" pullup on_press 50` |
...
## Infrastructure Modules (No CLI Commands)

These modules operate in the background to support the core system architecture. They do not have direct user-facing commands but are essential to the framework's operation.
